...
const APP_VERSION = "v1.0.7"; // UPDATES EVERYWHERE
...
function animateToLocation(gameX, gameY, targetZoom = null) {
    ...
    // FIX: Capture fitScale at animation start for consistent coordinate math
    ...
    // FIX: Use mapContainer dimensions instead of window for accurate center
...
function setZoomLevel(newLevel, mouseX = null, mouseY = null) {
  ...
  // FIX: Reset garrison zoom state if user manually zoomed out significantly
  if (zoomedGarrisonId !== null && state.scale < MAX_ZOOM - 0.5) {
    zoomedGarrisonId = null;
    // Don't reset preZoomScale - keep it for reference if they want to go back
    // Trigger UI refresh to update icon from - to +
    renderMarkers();
  }
      ...
      if (selectedGarrisonId === point.id) {
          ...
          const handleZoomToggle = (e) => {
              ...
              if (!isCurrentlyZoomed) {
                  ...
                  // Zoom to MAX_ZOOM (10x desktop, 20x mobile)
                  ...
                  // FIX: Only use saved pre-zoom if it's actually a zoom OUT from current
                  // If saved pre-zoom would zoom IN (higher than current), use fallback
